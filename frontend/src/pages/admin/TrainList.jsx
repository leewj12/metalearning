...
import styles from "/src/css/admin/TrainList.module.css";
...
const TrainList = () => {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(10); // 보여줄 게시물 목록 수
    ...
    const fetchTrainList = async () => {
      try {
        const response = await axios.get(`/api/admin/KDT/${kdtSessionId}/train/list`,{ // API 엔드포인트 수정
          ...
          ? [...response.data.trainList].sort((a, b) => new Date(b.kdtTrainDate) - new Date(a.kdtTrainDate))
          : [];

          setTrainList(sortedTrainList); // 정렬된 데이터로 설정
