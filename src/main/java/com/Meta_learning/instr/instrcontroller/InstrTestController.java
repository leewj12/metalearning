...
public class InstrTestController {
    ...
    // 시험 등록 폼 이동
    @GetMapping("/instr/KDT/{kdtSessionId}/test")
    public String getKdtTest(@PathVariable Long kdtSessionId, Model model) {
        ...
        return "instr/KDT/test";
    }


    //시험 상세 보기
    @GetMapping("/instr/KDT/{kdtSessionId}/test/{kdtTestId}")
    public String postKdtTest(@PathVariable Long kdtSessionId, @PathVariable Long kdtTestId, Model model,
        ...
        if(kdtTestDTO ==null){
            ...
            model.addAttribute("loc", "/view/instr/KDT/" + kdtSessionId + "/test/list");  // 시험 목록으로 이동
            return "utility/message";
        }

        // 본인이 작성한 시험이 아닌 경우
        if(!kdtTestDTO.getUserId().equals(user.getUserId())){
            model.addAttribute("msg", "시험을 출제한 강사만 조회가 가능합니다.");
            model.addAttribute("loc", "/view/instr/KDT/" + kdtSessionId + "/test/list");  // 시험 목록으로 이동
            return "utility/message";
        }
        ...
        return "instr/KDT/testdetail";
    }


    // 시험 수정 폼 이동
    @GetMapping("/instr/KDT/{kdtSessionId}/test/update/{kdtTestId}")
    public String getKdtTestUpdate(@PathVariable Long kdtSessionId, @PathVariable Long kdtTestId, Model model,
        ...
        // 본인이 작성한 시험이 아닌 경우
        if(!kdtTestDTO.getUserId().equals(user.getUserId())){
            model.addAttribute("msg", "본인이 작성한 시험만 수정 가능합니다.");
            model.addAttribute("loc", "/view/instr/KDT/" + kdtSessionId + "/test/list");  // 시험 목록으로 이동
            return "utility/message";
        }
        ...
        return "instr/KDT/testupdate";
    }

    // 답안 상세 폼 이동
    @GetMapping("/instr/KDT/{kdtSessionId}/test/submit/detail/{kdtTestId}/{kdtPartId}")
    public String getKdtTestSubmitDetail(@PathVariable Long kdtSessionId, @PathVariable Long kdtTestId, @PathVariable Long kdtPartId,
        ...
        if(kdtTestSubmitDTOS ==null || kdtTestSubmitDTOS.isEmpty()){
            ...
            model.addAttribute("loc", "/view/instr/KDT/" + kdtSessionId + "/test/submit/"+ kdtTestId+"/list");  // 시험 답안 목록으로 이동
            return "utility/message";
        }
        ...
        return "instr/KDT/testsubmitdetail";
    }

    // 채점 수점 폼 이동
    @GetMapping("/instr/KDT/{kdtSessionId}/test/grading/update/{kdtTestId}/{kdtPartId}")
    public String getKdtTestGradingUpdate(@PathVariable Long kdtSessionId, @PathVariable Long kdtTestId,
        ...
        // 본인이 작성한 시험이 아닌 경우
        if(!kdtTestDTO.getUserId().equals(user.getUserId())){
            model.addAttribute("msg", "시험을 출제한 강사만 채점 수정이 가능합니다.");
            model.addAttribute("loc", "/view/instr/KDT/" + kdtSessionId + "/test/list");  // 시험 목록으로 이동
            return "utility/message";
        }
        ...
        return "instr/KDT/testgradingupdate";
    }
