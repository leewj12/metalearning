...
const InstrPartList = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get(`/api/instr/KDT/${kdtSessionId}/part/list`,{ // API 엔드포인트 수정
          headers: {
   ...
   const handleSave = async () => {
    try {
      const response = await axios.put(
        `/api/instr/KDT/${kdtSessionId}/part/update/${editingUserId}`,
        {
      ...
      {currentUsers.length>0 ?(
        currentUsers.map((user, index) => (
            <tr key={user.userId}>
              ...
              <td><a href={`/instr/users/${user.userId}`}>상세보기</a></td>
              <td>
                {editingUserId === user.kdtPartId ? (
