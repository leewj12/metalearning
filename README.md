...
1. GitHub Actions 러너에서 프론트엔드 빌드 (`npm ci && npm run build`)
2. `./gradlew bootWar` 빌드
3. Docker 이미지를 빌드하여 `ghcr.io/leewj12/metalearning:latest`로 Push
4. SSH로 EC2 접속 → `git pull` → `docker compose pull` → `docker compose up -d`
