...
const InstrTestSubmit = () => {
  ...
  useEffect(() => {
    const fetchSubmitList = async () => {
      try {
        const response = await axios.get(`/api/instr/KDT/${kdtSessionId}/test/submit/${kdtTestId}/list`,{ // API 엔드포인트 수정
          headers: {
        ...
        <tbody>
          {currentItems.map((submit, index) => (
            <tr key={submit.kdtPartId}>
              ...
              <td>
                <a href={`/instr/KDT/${sessionInfo.kdtSessionId}/test/submit/detail/${testInfo.kdtTestId}/${submit.kdtPartId}`}>
                  {submit.kdtPartName}
                </a>
              </td>
...
export default InstrTestSubmit;